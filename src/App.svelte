    ...
    <article class="planets-list-area">
      ...
      <fieldset class= "planetsFieldset" style="display: {menuFieldset.displayPlanets}">
        ...
        <div class="planets-list">
          {#each allPlanetsOfSolarSystem as planet}
          <div class="${planet.name}" >
            <input type="checkbox" name="${planet.name}" bind:checked={planet.showClock} onchange={savingPlanet} id="${planet.name}"/>
            <label for="${planet.name}" id="${planet.name}">{planet.name}</label>
          </div>
          {/each}
        </div>
      </fieldset>
    ...
    <div class="planet-description" role="button" onclick={closePlanetInformationOut} onkeydown={keyDownPopUp} tabindex="0">
      ...
      <div class="inside-planet-description" role="document">
        ...
        <div class="pop-up-overlay">
          ...
          <a href={planetDescription.link} aria-label="link to wikipedia for ${planetDescription.name}" rel="noopener noreferrer" target="_blank">More information on Wikipedia</a>
          {/if}
          {/each}
        </div>
      </div>
